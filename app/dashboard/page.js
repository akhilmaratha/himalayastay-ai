          ...
          {loading ? (
            ...
            upcomingBookings.map((booking, idx) => (
              <article key={booking.id || idx} className="bg-surface rounded-xl ambient-shadow-1 border border-surface-container-highest overflow-hidden flex flex-col sm:flex-row group transition-all duration-300 hover:ambient-shadow-2">
                <div className="sm:w-100 h-48 sm:h-auto relative overflow-hidden">
                  ...
                  <img 
                    alt={booking.roomId?.title || "Property image"} 
                ...
                <div className="p-md flex flex-col justify-between sm:w-3/5 gap-sm">
                  <div>
                    <div className="flex justify-between items-start mb-xs">
                      <h3 className="font-headline-lg-mobile text-headline-lg-mobile text-primary">
                        {booking.roomId?.title || booking.roomId?.name || `Room ${booking.roomId?._id || booking.roomId}`}
                      </h3>
                    </div>
                    <p className="font-body-md text-body-md text-on-surface-variant mb-sm">{booking.roomId?.location || "Himalayas, India"}</p>
                  ...
                  <div className="flex gap-sm pt-sm border-t border-surface-container-highest">
                    <Link href={`/property/${booking.roomId?.slug || booking.roomId?._id || booking.roomId}`} className="flex-1 bg-primary text-on-primary font-label-md text-label-md py-xs px-sm rounded-lg hover:bg-tertiary-container transition-colors duration-200 text-center">
                      View Details
                    </Link>
              ...
              {pastBookings.map((booking, idx) => (
                <li key={booking.id || idx} className="flex flex-col sm:flex-row items-start sm:items-center p-md border-b border-surface-container-highest last:border-b-0 hover:bg-surface-container-low transition-colors duration-200 gap-md">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-body-md text-body-md text-on-surface-variant">{booking.roomId?.location || "Himalayas, India"}</p>
